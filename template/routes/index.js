	...
	if (name) {
		fs.exists('project/' + name + '/data.js', function (exists) {
			if (exists) {
				...
				/*
				new targz().compress('project/' + name, 'project/' + name + '.tar.gz', function(err){
					if (err) {
						console.log(err);
						res.send({result: false, error: '压缩项目失败'});
						return;
					}
					ftp = new JSFtp({
					  host: "192.168.1.105",
					...
					ftp.put('project/' + name + '.tar.gz', name + '.tar.gz', function (err) {
						if (err) {
							console.log(err);
							res.send({result: false, error: '上传失败'});
						}
