...
program
  ...
  .option('-s, --scafford [ProjectName] [pc || app]', '搭建项目原型',CreateApp,""+process.argv.slice(-1))
...
function setOpen(name) {
  ...
  if ( !isNaN(name) && name<arr.length) {
    arr = arr.filter(function(e){  return e.substr(-5) === '.html' })
...
function CreateApp(name,kind){
  projectName = name 
  var kind = kind?kind:'pc';
  fs.copy(__dirname + '/../template/'+kind, cwd + '/' + name + '/', function(err){
